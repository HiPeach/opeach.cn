// 控制台文字
console.log('██╗  ██╗██╗██████╗ ███████╗ █████╗  ██████╗██╗  ██╗\n██║  ██║██║██╔══██╗██╔════╝██╔══██╗██╔════╝██║  ██║\n███████║██║██████╔╝█████╗  ███████║██║     ███████║\n██╔══██║██║██╔═══╝ ██╔══╝  ██╔══██║██║     ██╔══██║\n██║  ██║██║██║     ███████╗██║  ██║╚██████╗██║  ██║\n╚═╝  ╚═╝╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝\nCopyright © 2023 HiPeach Inc. All Rights Reserved.');
...
window.addEventListener("scroll", function() {
  ...
  // 如果滚动位置为0，则页面在最顶部，更改元素的背景颜色
  if (scrollPosition === 0) {
    document.querySelector('#nav').style.backdropFilter = 'none';
  } else {
    // 否则，页面不在最顶部，将元素的背景颜色设置为默认值
    document.querySelector('#nav').style.backdropFilter = 'saturate(180%) blur(20px)';
  }
});
